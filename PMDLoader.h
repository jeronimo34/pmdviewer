...
namespace MmdStruct{
  ...
  struct PmdVertex{
    ...
    float pos[3]; // x, y, z // 蠎ｧ讓・	

	float normal_vec[3]; // nx, ny, nz // 豕慕ｷ壹・繧ｯ繝医Ν
    
	float uv[2]; // u, v // UV蠎ｧ讓・// MMD縺ｯ鬆らせUV
	
	WORD bone_num[2]; // 繝懊・繝ｳ逡ｪ蜿ｷ1縲∫分蜿ｷ2 // 繝｢繝・Ν螟牙ｽ｢(鬆らせ遘ｻ蜍・譎ゅ↓蠖ｱ髻ｿ
	
	BYTE bone_weight; // 繝懊・繝ｳ1縺ｫ荳弱∴繧句ｽｱ髻ｿ蠎ｦ // min:0 max:100 // 繝懊・繝ｳ2縺ｸ縺ｮ蠖ｱ髻ｿ蠎ｦ縺ｯ縲・100 - bone_weight)
	
	BYTE edge_flag;

	// 0:騾壼ｸｸ縲・:繧ｨ繝・ず辟｡蜉ｹ // 繧ｨ繝・ず(霈ｪ驛ｭ)縺梧怏蜉ｹ縺ｮ蝣ｴ
  };
  ...
  struct PmdMorph{
    char skin_name[20]; //縲陦ｨ諠・錐
    
	DWORD skin_vert_count; // 陦ｨ諠・畑縺ｮ鬆らせ謨ｰ
